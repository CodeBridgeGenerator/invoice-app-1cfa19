...
const [itemId, setItemId] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            companyId: _entity?.companyId?._id,itemId: _entity?.itemId?._id,quantity: _entity?.quantity,subTotal: _entity?.subTotal,discount: _entity?.discount,total: _entity?.total,
